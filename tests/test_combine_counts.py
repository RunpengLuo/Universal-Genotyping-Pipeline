...
Targets the two functions that decide bin-level BAF correctness:

- `aggregation_utils.matrix_segmentation`: sum SNP-by-sample counts into
  bin-by-sample counts.
- `phasing_utils.detect_phase_flips`: split SNPs into phase_groups at
  haplotype-orientation switches so a bin never sums across a flip.

Bin BAF is `sum(B) / sum(total)` over a bin's SNPs, which is only correct when the
...
`phasing_utils` needs only numpy/pandas/scipy, so the phase-flip tests run anywhere
that stack exists; `matrix_segmentation` still needs numba, so the tests using it take
the `agg` fixture and skip on their own when it is absent.
...
@pytest.fixture
def agg():
    """aggregation_utils, skipping the test when its numba dependency is absent."""
...
def test_matrix_segmentation_sums_per_bin(agg):
    x = np.array([[3.0, 0.0], [1.0, 0.0], [2.0, 5.0]])
    out = _dense(agg.matrix_segmentation(x, np.array([0, 0, 1]), 2))
    assert out.tolist() == [[4.0, 0.0], [2.0, 5.0]]


def test_same_orientation_bin_baf_no_cancellation(agg):
    # 3 LOH SNPs, all B-skewed (BAF 0.8) -> bin BAF stays 0.8
    b = np.array([[8.0], [7.0], [9.0]])
    tot = b + np.array([[2.0], [3.0], [1.0]])
    bins = np.array([0, 0, 0])
    baf = (
        _dense(agg.matrix_segmentation(b, bins, 1))[0, 0]
        / _dense(agg.matrix_segmentation(tot, bins, 1))[0, 0]
    )
...
def test_detect_flip_at_orientation_switch():
    ...
    pg = ph.detect_phase_flips(_snps(6), a, b, grp_cols=["region_id", "PS"]).to_numpy()
...
def test_no_flip_when_balanced():
    # BAF ~0.5 everywhere -> no orientation to switch -> single phase_group
    b = np.full((6, 1), 50.0)
    a = np.full((6, 1), 50.0)
    pg = ph.detect_phase_flips(_snps(6), a, b, grp_cols=["region_id", "PS"]).to_numpy()
...
def test_loh_cancelled_when_merged_but_preserved_when_split(agg):
    ...
    merged = (
        _dense(agg.matrix_segmentation(b, np.zeros(6, int), 1))[0, 0]
        / _dense(agg.matrix_segmentation(tot, np.zeros(6, int), 1))[0, 0]
    )
    ...
    k = int(bins.max()) + 1
    bsum = _dense(agg.matrix_segmentation(b, bins, k))[:, 0]
    tsum = _dense(agg.matrix_segmentation(tot, bins, k))[:, 0]
