...
Targets the two functions that decide bin-level BAF correctness in
`workflow/scripts/aggregation_utils.py`:

- `matrix_segmentation`: sum SNP-by-sample counts into bin-by-sample counts.
- `detect_phase_flips`: split SNPs into phase_groups at haplotype-orientation
  switches so a bin never sums across a flip.
...
Needs the base.yaml scientific stack (numba/scanpy/...); skipped otherwise.
Run in that env:  python -m pytest tests/test_combine_counts.py -v
"""
...
def test_detect_flip_at_orientation_switch():
    ...
    pg = agg.detect_phase_flips(_snps(6), a, b, grp_cols=["region_id", "PS"]).to_numpy()
...
def test_loh_cancelled_when_merged_but_preserved_when_split():
    ...
    merged = _dense(agg.matrix_segmentation(b, np.zeros(6, int), 1))[0, 0] / _dense(
        agg.matrix_segmentation(tot, np.zeros(6, int), 1)
    )[0, 0]
